...
void *increment(void *args) {
    ...
    for (int i = 0; i < data->args->iterations; i++) {
        ...
        mtx_lock(data->args->iterMtx);

        if(data->args->currentIterations >= data->args->iterations){

            mtx_unlock(data->args->iterMtx);
            free(args);
            return NULL;
        }
        data->args->currentIterations = data->args->currentIterations +1;
...
int main(int argc, char **argv) {
    ...
    args.arr = &arr;
    args.lastId = 0;
    args.currentIterations = 0;
    mtx_t mtxs[arr.size];
    mtx_t iterMtx;

    args.mutex = mtxs;
    args.iterMtx = &iterMtx;
